...
class TestBook:
    ...
    #     book = Book('1INCH-USDC', simTime, r'E:\out3\books\1INCH-USDC-400', 10000)
    ...
    def test_case2(self):
        ...
        # 1000
        simTime.set(1000)
        assert book.asks[0] == (1011, 1, 1)
        assert book.asks[1] == (1012, 1, 1)
        assert book.asks[2] == (1013, 1, 1)
        assert book.asks[3] == (1014, 1, 1)
        assert book.asks[4] == (1015, 1, 1)
        
        assert book.bids[0] == (1009, 1, 1)
        assert book.bids[1] == (1008, 1, 1)
        assert book.bids[2] == (1007, 1, 1)
        assert book.bids[3] == (1006, 1, 1)
        assert book.bids[4] == (1005, 1, 1)
        
        # 2000
        simTime.set(2000)
        assert book.asks[0] == (1020.9, 1, 1)
        assert book.asks[1] == (1021.9, 1, 1)
        assert book.asks[2] == (1022.9, 1, 1)
        assert book.asks[3] == (1023.9, 1, 1)
        assert book.asks[4] == (1024.9, 1, 1)
        
        assert book.bids[0] == (1018.9, 1, 1)
        assert book.bids[1] == (1017.9, 1, 1)
        assert book.bids[2] == (1016.9, 1, 1)
        assert book.bids[3] == (1015.9, 1, 1)
        assert book.bids[4] == (1014.9, 1, 1)
        
        # 3000
        simTime.set(3000)
        assert book.asks[0] == (1030.6, 1, 1)
        assert book.asks[1] == (1031.6, 1, 1)
        assert book.asks[2] == (1032.6, 1, 1)
        assert book.asks[3] == (1033.6, 1, 1)
        assert book.asks[4] == (1034.6, 1, 1)
        
        assert book.bids[0] == (1028.6, 1, 1)
        assert book.bids[1] == (1027.6, 1, 1)
        assert book.bids[2] == (1026.6, 1, 1)
        assert book.bids[3] == (1025.6, 1, 1)
        assert book.bids[4] == (1024.6, 1, 1)
        
        # 4000
        simTime.set(4000)
        assert book.asks[0] == (1039.9, 1, 1)
        assert book.asks[1] == (1040.9, 1, 1)
        assert book.asks[2] == (1041.9, 1, 1)
        assert book.asks[3] == (1042.9, 1, 1)
        assert book.asks[4] == (1043.9, 1, 1)
        
        assert book.bids[0] == (1037.9, 1, 1)
        assert book.bids[1] == (1036.9, 1, 1)
        assert book.bids[2] == (1035.9, 1, 1)
        assert book.bids[3] == (1034.9, 1, 1)
        assert book.bids[4] == (1033.9, 1, 1)
        
        # 16000
        simTime.set(16000)
        assert book.asks[0] == (1101, 1, 1)
        assert book.asks[1] == (1102, 1, 1)
        assert book.asks[2] == (1103, 1, 1)
        assert book.asks[3] == (1104, 1, 1)
        assert book.asks[4] == (1105, 1, 1)
        
        assert book.bids[0] == (1099, 1, 1)
        assert book.bids[1] == (1098, 1, 1)
        assert book.bids[2] == (1097, 1, 1)
        assert book.bids[3] == (1096, 1, 1)
        assert book.bids[4] == (1095, 1, 1)
